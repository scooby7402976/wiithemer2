<?php
	...
	if(isset($_POST["action"])) {
		...
		switch($action) {
			...
			case "find_current":{
				$current_users = 0;
				if ($dh = opendir("C:\Apache24\server\wiithemer")){
					while (($file = readdir($dh)) !== false){
						...
						else if($file == "deletesessionfolders.ahk") continue;
						...
						else if($file == "jquery-3.7.1.js") continue;
						else $current_users++;
					}
					closedir($dh);
					echo $current_users + 1;
				}
			}break; 
		}
		return;
	...
	function add_mym_Extension($theme_Selected) {
		if((($theme_Selected >= 32) && $theme_Selected <= 39 ) || ($theme_Selected == 68) || ($theme_Selected == 179))
			return true;
		return false;
	}
